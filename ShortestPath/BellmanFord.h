/**
 *  Created by zhuozhu on 2022/4/30 19:22.
 *  Description: BellmanFord.h
 */
//
...
public:
    BellmanFord(Graph &graph, int s) : G(graph), s(s) {
        ...
        distTo[s] = Weight();  // 设置distTo[s] = 0, 并且让from[s]不为NULL, 表示初始s节点可达且距离为0
        from[s] = new Edge<Weight>(s, s, Weight()); // 这里from[s]的内容是new出来的, 注意要在析构函数里delete掉
